...
class Uid2PublisherClient:
    """Client for interacting with UID2 publisher services.
        ...
        Examples:
            ...
            >>> client = Uid2PublisherClient('https://prod.uidapi.com', 'my-authorization-key', 'my-secret-key', IdentityScope.UID2)
    ...
    def generate_token(self, token_generate_input):
        req, nonce = make_v2_request(self._secret_key, dt.datetime.now(tz=timezone.utc), token_generate_input.get_as_json_string().encode())
    ...
    def refresh_token(self, current_identity):
        resp = post(self._base_url, '/v2/token/refresh', headers=auth_headers(self._auth_key), data=current_identity.get_refresh_token().encode())
